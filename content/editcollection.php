...
// if artist countries are supported get all country codes associated with 
// artists from each endpoint
if ($collection->hascapability("artistcountry")) {
	$countries = array();
	foreach ($collection->endpoints() as $ep) {
		$epcountries = $ep->query("
...
if (isset($_REQUEST["submit"])) {
	...
	// artist name
	if ($collection->hascapability("artistname") && isset($collection->data["filteractive_artistname"]) && isset($collection->data["artistname_regex"]) && !empty($collection->data["artistname_regex"]))
	...
	// artist country
	if ($collection->hascapability("artistcountry") && isset($collection->data["filteractive_artistcountry"]) && isset($collection->data["artistcountry_countrycode"]) && is_array($collection->data["artistcountry_countrycode"])) {
		$unions = array();
	...
	// record date
	if ($collection->hascapability("recorddate") && isset($collection->data["filteractive_recorddate"])) {
		// only way i can find to compare against xsd:date is to just to a 
		// string comparison -- xsd:dateTime is easier but this works as a 
		// catch-all.
		if (isset($collection->data["recorddate_from_active"]) && preg_match("%^[0-9]{4}-[0-9]{2}-[0-9]{2}$%", $collection->data["recorddate_from"]))
			$subfilters[] = "str(?recorddate) >= \"" . $collection->data["recorddate_from"] . "\"";
		if (isset($collection->data["recorddate_before_active"]) && preg_match("%^[0-9]{4}-[0-9]{2}-[0-9]{2}$%", $collection->data["recorddate_before"]))
			$subfilters[] = "str(?recorddate) < \"" . $collection->data["recorddate_before"] . "\"";
		if (!empty($subfilters))
			$collection->addfilter("
	...
	// record name
	if ($collection->hascapability("recordname") && isset($collection->data["filteractive_recordname"]) && isset($collection->data["recordname_regex"]) && !empty($collection->data["recordname_regex"]))
		$collection->addfilter("
	...
	// track name
	if ($collection->hascapability("trackname") && isset($collection->data["filteractive_trackname"]) && isset($collection->data["trackname_regex"]) && !empty($collection->data["trackname_regex"]))
		$collection->addfilter("
	...
	// track number
	if ($collection->hascapability("tracknumber") && isset($collection->data["filteractive_tracknumber"]) && isset($collection->data["tracknumber_number"]) && !empty($collection->data["tracknumber_number"]))
		$collection->addfilter("
	...
	$collection->query(prefix(array("geo", "foaf", "mo", "tags", "dc", "xsd")) . "
		SELECT * WHERE {
			...
			" . implode("\n\t\t", $collection->filters(array("record", "track", "trackname", "tracknumber", "recorddate", "recordcreated", "recordname"), $filters)) . "
			...
			" . implode("\n\t\t", $collection->filters(array("trackname", "recorddate", "trackcreated", "tracknumber"), $filters)) . "
...
<div class="trythis collapsed">
	<div class="content">
		<p>From this page you can build a new collection of songs.</p>
...
<form action="<?php echo SITEROOT_WEB; ?>editcollection/<?php echo $collection->id(); ?>" method="post">
	<dl>
		...
		<dd>
			<ul>
				<?php foreach ($collection->endpoints() as $ep) { ?>
					<li><?php echo htmlspecialchars($ep->name()); ?></li>
				<?php } ?>
			</ul>
		</dd>
		...
		<dt>Filters</dt>
		<dd>
			<?php if (count($collection->endpointcapabilities()) == 0) { ?>
				<p>No filters are available for the selected endpoints.</p>
			<?php } else { ?>
				<dl id="filters">
					<?php if ($collection->hascapability("artistname")) { ?>
						<dt>
							<label>
					...
					<?php if ($collection->hascapability("artistcountry")) { ?>
						<dt>
							<label>
					...
					<?php if ($collection->hascapability("recorddate")) { ?>
						<dt>
							<label>
					...
					<?php if ($collection->hascapability("recordname")) { ?>
						<dt>
							<label>
					...
					<?php if ($collection->hascapability("recordtag")) { ?>
						<dt>
							<label>
					...
					<?php if ($collection->hascapability("trackname")) { ?>
						<dt>
							<label>
					...
					<?php if ($collection->hascapability("tracknumber")) { ?>
						<dt>
							<label>
		...
		<dd>
			...
			Limit the number of results to 250 per endpoint
				...
				<?php foreach ($collection->results() as $result) { ?>
					<tr>
						...
						<td>
							...
							$date = null;
							$type = null;
							if (isset($result["recorddate type"])) {
								$date = $result["recorddate"];
								$type = $result["recorddate type"];
							}
							if ($type == "literal")
								echo htmlspecialchars(date("Y-m-d", strtotime($date)));
							else
								echo "got a type '$type' and value '$date'";
							?>
						</td>
